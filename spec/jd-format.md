...
An implementation conforms to this specification if it correctly implements the syntax, semantics, options system, and error handling defined herein. Implementations SHOULD pass the test suite provided in `test/` and document any limitations or extensions.
...
Examples are provided in [examples.md](examples.md), including:
...
The test suite in `test/` provides validation for implementations.
...
5. **Passes** the test suite

Implementations SHOULD document any extensions or limitations.
